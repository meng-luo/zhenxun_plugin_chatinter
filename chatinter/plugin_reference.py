...
def _is_exact_schema_match(schema: PluginCommandSchema, query: str) -> bool:
    normalized = normalize_message_text(query).casefold()
    if not normalized:
        return False
    candidates = [
        normalize_message_text(schema.head).casefold(),
        *[
            normalize_message_text(alias).casefold()
            for alias in schema.aliases
            if normalize_message_text(alias)
...
def _select_prompt_schemas(
    ...
    *,
    ...
    selected: list[PluginCommandSchema] = []
    for _score, _index, schema in scored:
        if _is_exact_schema_match(schema, query) and schema not in selected:
            selected.append(schema)
            if len(selected) >= limit:
                break
    for score, _index, schema in scored:
        if len(selected) >= limit:
            break
        if score > 0 and schema not in selected:
            selected.append(schema)
    if selected:
        return selected
    return [schema for _score, _index, schema in scored[:limit]]

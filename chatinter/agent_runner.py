...
from .config import (
    AGENT_ABORT_PARALLEL_ON_FAILURE,
    AGENT_DIMINISHING_TOKEN_DELTA,
    AGENT_EXPAND_TOOLS_STEP,
    AGENT_PARALLEL_SAFE_TOOLS,
...
async def run_chatinter_agent(
    *,
    ...
    async def _run() -> LLMResponse:
        ...
        parallel_safe_tools = max(int(AGENT_PARALLEL_SAFE_TOOLS), 1)
        abort_parallel_on_failure = AGENT_ABORT_PARALLEL_ON_FAILURE
        agent_token_budget = max(int(AGENT_TOKEN_BUDGET), 0)
        diminishing_token_delta = max(int(AGENT_DIMINISHING_TOKEN_DELTA), 40)

...
def resolve_thread_context(
    *,
    ...
    if event_context.reply and event_context.reply.message_id:
        ...
        ctx = ThreadContext(
            ...
            related_user_ids=tuple(
                user_id
                for user_id in (event_context.user_id, event_context.reply.sender_id)
                if user_id
            ),
        )
    ...
    ctx = ThreadContext(
        thread_id=_stable_thread_id(seed),
        source="topic",
        confidence=0.55 if topic_key else 0.35,
        related_user_ids=tuple(
            user_id
            for user_id in (event_context.user_id, addressee.target_user_id)
            if user_id
        ),
    )
    _thread_cache[ctx.thread_id] = (now, ctx)
